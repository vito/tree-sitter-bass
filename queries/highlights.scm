...
(
  (list
    .
    (symbol) @conditional
    (#match? @conditional "^(if|case|cond|when)$"))
  (set! "priority" 105)
)
(
  (cons
...
(
  (list
    ...
    (#match? @function.macro "^(op|fn|current-scope|quote|let|provide|module|or|and|->|curryfn|for|\\$|linux)$"))
  (set! "priority" 105)
)
(
  (cons
